...
def test_ui_C6178891_try_change_password_with_invalid_current_password_and_not_equals_new_password(open_browser):
    ...
    ProfileSecurityPage(open_browser).enter_confirm_new_password(valid_password_autofirstnametest)

    # Клик на пусток поле
    ProfileSecurityPage(open_browser).click_field_current_password()

    # Проверка появления сообщения "Пароли не совподают"
    ProfileSecurityPage(open_browser).check_message_mismatch_password()
