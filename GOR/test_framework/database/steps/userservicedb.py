...
class StepsUserService:
    ...
    def delete_fingerprint_from_table_by_id_client(self, client_id) -> Any:
        fingerprint = self.queries.find_fingerprint_by_client_id_from_table_fingerprint(client_id=client_id)
        if fingerprint:
            self.queries.delete_fingerprint_by_client_id_from_table_fingerprint(fingerprint)
